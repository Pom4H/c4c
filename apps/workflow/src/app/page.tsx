...
function hydrateExecutionResult(result: SerializedWorkflowExecutionResult): WorkflowExecutionResult {
	...
	let hydratedError: Error | undefined;
	if (error) {
		hydratedError = new Error(error.message);
		hydratedError.name = error.name ?? "Error";
		if (error.stack) {
			hydratedError.stack = error.stack;
		}
	}
	return {
		...rest,
...
export default function Home() {
  ...
  const workflowStreamBase = `${workflowApiBase}/executions`;

  const { workflows, fetchWorkflows } = useWorkflows({
    apiBaseUrl: workflowApiBase,
    listPath: "/definitions",
  });
  const { definition, fetchDefinition } = useWorkflowDefinition(selectedWorkflowId, {
    apiBaseUrl: workflowApiBase,
    definitionPath: (id) => `/definitions/${encodeURIComponent(id)}`,
  });
  const { execute: executeWorkflowRun, isExecuting, error } = useWorkflow({
    apiBaseUrl: workflowApiBase,
    onSuccess: (res) => {
      setFinalResult(res);
      setActiveTab("graph");
  ...
  const handleExecute = async () => {
    ...
    // trigger execution through hook for compatibility
    const executionId = `wf_ui_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;

    const streamUrl = `${workflowStreamBase}/${encodeURIComponent(executionId)}/stream`;
    const es = new EventSource(streamUrl);
    eventSourceRef.current = es;

    executeWorkflowRun(selectedWorkflowId, {}, { executionId }).catch((err) => {
    ...
    const onEvent = (evt: MessageEvent) => {
      try {
        ...
        if (data.type === "workflow.result") {
          ...
          setFinalResult(hydrateExecutionResult(data.result));
          if (eventSourceRef.current) {
            eventSourceRef.current.close();
            eventSourceRef.current = null;
          }
