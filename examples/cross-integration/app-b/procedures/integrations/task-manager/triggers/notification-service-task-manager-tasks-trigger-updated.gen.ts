// This file is auto-generated by c4c integrate command
// Do not edit manually.

import type { Procedure, Contract } from "@c4c/core";
import { z } from "zod";

export const NotificationServiceTaskManagerTasksTriggerUpdatedContract: Contract = {
  name: "task-manager.notification.service.task.manager.tasks.trigger.updated",
  description: "Webhook fired when a task is updated",
  input: z.unknown(),
  output: z.record(z.string(), z.unknown()),
  metadata: {
    exposure: "external" as const,
    roles: ["workflow-node"],
    provider: "task-manager",
    operation: "notificationServiceTaskManagerTasksTriggerUpdated",
    tags: ["task-manager", "webhook"],
    type: "trigger" as const,
    trigger: {
      type: "webhook",
    },
  },
};

// Webhook triggers don't have a handler - they are registered as event receivers
export const NotificationServiceTaskManagerTasksTriggerUpdatedProcedure: Procedure = {
  contract: NotificationServiceTaskManagerTasksTriggerUpdatedContract,
  handler: async () => {
    throw new Error('Webhook triggers should not be called directly - they are invoked by the workflow engine');
  },
};
