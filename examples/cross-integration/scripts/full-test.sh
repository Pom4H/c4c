...
if curl -sf http://localhost:3001/openapi.json > /dev/null; then
  # Count paths in OpenAPI spec without jq
  PROCEDURES_A=$(curl -s http://localhost:3001/openapi.json | grep -o '"/rpc/[^"]*"' | wc -l)
...
if curl -sf http://localhost:3002/openapi.json > /dev/null; then
  # Count paths in OpenAPI spec without jq
  PROCEDURES_B=$(curl -s http://localhost:3002/openapi.json | grep -o '"/rpc/[^"]*"' | wc -l)
