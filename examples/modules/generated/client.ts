...
function resolveOptions(options: c4cClientOptions = {}): ResolvedClientOptions {
...
export function createc4cClient(options: c4cClientOptions = {}) {
  ...
  const invoke = async <P extends keyof ProcedureDefinitions>(
    ...
    if (!response.ok) {
      ...
      throw new Error(`RPC request to ${String(name)} failed — ${message}`);
    }
    return (await response.json()) as ProcedureDefinitions[P]["output"];
  };
  const procedures = {
    "analytics.stats": (input: AnalyticsStatsInput) => invoke("analytics.stats", input),
    "analytics.health": (input: AnalyticsHealthInput) => invoke("analytics.health", input),
    "products.create": (input: ProductsCreateInput) => invoke("products.create", input),
    "products.get": (input: ProductsGetInput) => invoke("products.get", input),
    "products.list": (input: ProductsListInput) => invoke("products.list", input),
    "products.updateStock": (input: ProductsUpdateStockInput) => invoke("products.updateStock", input),
    "users.create": (input: UsersCreateInput) => invoke("users.create", input),
    "users.delete": (input: UsersDeleteInput) => invoke("users.delete", input),
    "users.get": (input: UsersGetInput) => invoke("users.get", input),
    "users.list": (input: UsersListInput) => invoke("users.list", input),
    "users.update": (input: UsersUpdateInput) => invoke("users.update", input),
  } as const;
  return { invoke, procedures };
}

export type c4cClient = ReturnType<typeof createc4cClient>;
