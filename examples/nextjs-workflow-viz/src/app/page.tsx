...
import type { WorkflowDefinition, WorkflowExecutionResult } from "@/lib/workflow/types";
import { useEffect, useState } from "react";
import { executeWorkflowAction, getAvailableWorkflows, getWorkflowDefinition } from "./actions";
			...
			<div className="max-w-7xl mx-auto animate-slide-in">
							...
							<div className="flex items-center gap-2">
								...
								<div>
									...
									<div className="text-sm text-muted-foreground">
										Duration: {executionResult.executionTime}ms | Nodes executed:{" "}
										{executionResult.nodesExecuted.length} | Spans collected:{" "}
				...
				<div className="workflow-card overflow-hidden">
					<div className="border-b border-border">
						<nav className="flex">
							<button
								...
								className={`workflow-tab ${activeTab === "graph" ? "workflow-tab-active" : ""}`}
							>
								📊 Workflow Graph
							</button>
							<button
								onClick={() => setActiveTab("gantt")}
								className={`workflow-tab ${activeTab === "gantt" ? "workflow-tab-active" : ""}`}
							>
								📈 Span Gantt Chart
							</button>
							<button
								onClick={() => setActiveTab("trace")}
								className={`workflow-tab ${activeTab === "trace" ? "workflow-tab-active" : ""}`}
							>
								🔍 Trace Details
							</button>
						</nav>
					...
					<div className="p-6">
						...
						{activeTab === "gantt" && <SpanGanttChart spans={executionResult?.spans || []} />}

						{activeTab === "trace" && <TraceViewer spans={executionResult?.spans || []} />}
					</div>
				</div>
				...
				<div className="mt-8 text-center text-muted-foreground text-sm">
					<p>Built with Next.js 15, React Flow, and OpenTelemetry Protocol</p>
					<p className="mt-1">
						Server Actions execute workflows and collect traces for visualization
					</p>
				</div>
			</div>
		</main>
