		...
		<div className="space-y-4">
			<div className="flex items-center justify-between mb-4">
				...
				<div className="text-sm text-gray-600">
					Total Duration: {timelineMetrics.totalDuration}ms | Spans: {spans.length}
				</div>
			</div>
					...
					{flatSpans.map((row, idx) => {
						...
						return (
							<div
								...
								onClick={() => setSelectedSpan(selectedSpan === span.spanId ? null : span.spanId)}
							>
								{/* Span name column */}
								<div
								...
								>
									...
									<div className="flex-1 min-w-0">
										...
										<div className="text-xs text-gray-500">{span.duration}ms</div>
									</div>
								</div>
									...
									<div className="relative h-8 flex items-center">
										<div
											className={`absolute h-6 rounded shadow-sm transition-all ${getSpanColor(
												span
											)} ${isHovered || isSelected ? "ring-2 ring-blue-400 z-10 scale-105" : ""}`}
											style={{
												left: `${leftPercent}%`,
												width: `${widthPercent}%`,
												minWidth: "4px",
				...
				<div className="mt-4 p-4 bg-white border border-gray-300 rounded-lg shadow-sm">
					...
					<div className="grid grid-cols-2 gap-4 mb-4">
						...
						<div>
							...
							<div
								className={`font-semibold ${
									selectedSpanData.status.code === "OK" ? "text-green-600" : "text-red-600"
								}`}
							>
								{selectedSpanData.status.code}
								{selectedSpanData.status.message && ` - ${selectedSpanData.status.message}`}
							</div>
						</div>
						<div>
						...
						<div>
							...
							<div className="font-mono text-xs">{selectedSpanData.traceId}</div>
						</div>
						{selectedSpanData.parentSpanId && (
							<div>
								<div className="text-sm text-gray-600">Parent Span</div>
								<div className="font-mono text-xs">{selectedSpanData.parentSpanId}</div>
							</div>
						)}
					</div>
							...
							<div className="space-y-2">
								{selectedSpanData.events.map((event, idx) => (
									<div key={idx} className="bg-gray-50 p-3 rounded">
										...
										<div className="text-xs text-gray-600">Timestamp: {event.timestamp}</div>
										{event.attributes && (
			...
			<div className="mt-4 grid grid-cols-4 gap-4">
				...
				<div className="bg-purple-50 p-4 rounded-lg">
					<div className="text-2xl font-bold text-purple-700">
						{Math.round(spans.reduce((sum, s) => sum + s.duration, 0) / spans.length)}
						ms
					</div>
