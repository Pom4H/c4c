...
import type { TraceSpan, WorkflowDefinition } from "@/lib/workflow/types";
...
export default function WorkflowVisualizer({ workflow, executionResult }: WorkflowVisualizerProps) {
	...
	const initialNodes = useMemo(() => {
		...
		workflow.nodes.forEach((node, index) => {
			...
			const nodeSpan = executionResult?.spans.find((s) => s.attributes["node.id"] === node.id);
				...
				data: {
					label: (
						<div className="text-center">
							...
							{nodeSpan && <div className="text-xs mt-1 text-blue-600">{nodeSpan.duration}ms</div>}
						</div>
					),
				},
				style: {
					background: isExecuted ? nodeColors[node.type] || "#60a5fa" : "#e5e7eb",
			...
			if (node.next) {
				...
				nextNodes.forEach((nextId, index) => {
					edges.push({
						...
						style: {
							stroke: executionResult?.nodesExecuted.includes(node.id) ? "#4ade80" : "#94a3b8",
							strokeWidth: 2,
						},
					...
					<Panel position="top-right">
						<div className="bg-white p-4 rounded-lg shadow-lg max-w-xs">
							...
							<div className="text-sm space-y-1">
								<div>
									Nodes executed:{" "}
									<span className="font-semibold">{executionResult.nodesExecuted.length}</span>
								</div>
								<div>
									Spans collected:{" "}
									<span className="font-semibold">{executionResult.spans.length}</span>
								</div>
							</div>
						</div>
					</Panel>
...
function calculateNodeLevel(
	...
	const maxParentLevel = Math.max(...parents.map((p) => calculateNodeLevel(p.id, workflow, cache)));
