...
export function createWorkflowRouter(
  ...
  const workflowsPath =
    options.workflowsPath ?? process.env.c4c_WORKFLOWS_DIR ?? "workflows";
  ...
  router.get("/workflow/definitions", async (c) => {
    ...
    const workflows = library.summaries.map((summary) =>
      summarizeWorkflow(summary)
    );
  ...
  router.post("/workflow/execute", async (c) => {
    try {
      ...
      if (!workflow && body.workflowId) {
        workflow = await loadWorkflowDefinitionById(
          workflowsPath,
          body.workflowId
        );
        if (!workflow) {
  ...
  router.get("/workflow/executions/:executionId/stream", async (c) => {
    ...
    return streamSSE(c, async (stream) => {
      ...
      const unsubscribe = subscribeToExecution(executionId, async (event) => {
  ...
  router.get("/workflow/executions-stream", async (c) => {
    return streamSSE(c, async (stream) => {
      ...
      const unsubscribe = subscribeToAllExecutions(async (event) => {
        if (!open) return;
...
<body>
	...
	<p class="caption">${
    uiData.nodes.length
  } workflow-capable procedures · generated ${escapeHtml(
...
function buildWorkflowUIPayload(registry: Registry): WorkflowUIPayload {
  ...
  for (const [name, procedure] of registry.entries()) {
    ...
    const rpcOperation = (
      openapi.paths?.[rpcPath] as Record<string, unknown> | undefined
    )?.post as Record<string, any> | undefined | null;
    const inputSchema =
      rpcOperation?.requestBody?.content?.["application/json"]?.schema ??
      undefined;
    const outputSchema =
      rpcOperation?.responses?.["200"]?.content?.["application/json"]?.schema ??
      undefined;
