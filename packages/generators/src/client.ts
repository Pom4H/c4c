...
export function generateRpcClientModule(
	...
	lines.push("export interface c4cClientOptions {");
	...
	lines.push("function resolveOptions(options: c4cClientOptions = {}): ResolvedClientOptions {");
	...
	lines.push("export function createc4cClient(options: c4cClientOptions = {}) {");
	...
	lines.push("      throw new Error(`RPC request to ${String(name)} failed — ${message}`);");
	lines.push("    }");
	lines.push("    return (await response.json()) as ProcedureDefinitions[P][\"output\"];");
	lines.push("  };");
	lines.push("  const procedures = {");
	if (procedures.length === 0) {
		lines.push("    // No procedures available.");
	} else {
		for (const procedure of procedures) {
			lines.push(
				`    "${procedure.name}": (input: ${procedure.inputTypeName}) => invoke("${procedure.name}", input),`
			);
		}
	}
	lines.push("  } as const;");
	lines.push("  return { invoke, procedures };");
	lines.push("}\n");

	lines.push("export type c4cClient = ReturnType<typeof createc4cClient>;\n");
